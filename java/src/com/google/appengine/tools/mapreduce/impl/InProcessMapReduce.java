...
/**
 * Runs a MapReduce in the current process.  Only for very small datasets.
 * Easier to debug than a parallel MapReduce.
 *
...
public class InProcessMapReduce {
  ...
  private static class MapResult<K, V> {
    ...
    public MapResult(List<List<KeyValue<K, V>>> mapShardOutputs,
  ...
  private static class Impl<I, K, V, O, R> {
    ...
    private MapResult<K, V> map(List<? extends InputReader<I>> inputs) throws IOException {
      ...
      InMemoryOutput<KeyValue<K, V>> output = InMemoryOutput.create(inputs.size());
      List<? extends OutputWriter<KeyValue<K, V>>> outputs = output.createWriters();
      ...
      for (int shard = 0; shard < inputs.size(); shard++) {
        WorkerShardTask<I, KeyValue<K, V>, MapperContext<K, V>> task =
            new MapShardTask<I, K, V>(id, shard, inputs.size(),
                inputs.get(shard),
                mrSpec.getMapper(),
                outputs.get(shard),
                Long.MAX_VALUE);
      ...
      WorkerResult<KeyValue<K, V>> result =
          InProcessShardedJobRunner.
              runJob(tasks.build(),
                  new AbstractWorkerController<I, KeyValue<K, V>, MapperContext<K, V>>(
                    ...
                    @Override public void completed(
                        WorkerResult<KeyValue<K, V>> finalCombinedResult) {
                      counters[0] = finalCombinedResult.getCounters();
                    }
                  });
      log.info("Map phase completed");
      List<List<KeyValue<K, V>>> pairs = output.finish(outputs);
      return new MapResult<K, V>(pairs, result.getCounters());
    }

    private List<List<KeyValue<K, List<V>>>> shuffle(List<List<KeyValue<K, V>>> mapperOutputs,
        int reduceShardCount) {
      log.info("Shuffle phase started");
      List<List<KeyValue<K, List<V>>>> out = Shuffling.shuffle(mapperOutputs,
          mrSpec.getIntermediateKeyMarshaller(), reduceShardCount);
    ...
    InputReader<KeyValue<K, ReducerInput<V>>> getReducerInputReader(
      ...
      return new InputReader<KeyValue<K, ReducerInput<V>>>() {
        ...
        @Override public KeyValue<K, ReducerInput<V>> next() {
          ...
          KeyValue<K, ReducerInput<V>> result = KeyValue.of(data.get(i).getKey(),
              ReducerInputs.fromIterable(data.get(i).getValue()));
          i++;
          return result;
        }
      };
    ...
    private MapReduceResult<R> reduce(List<List<KeyValue<K, List<V>>>> inputs,
        Output<O, R> output, List<? extends OutputWriter<O>> outputs,
        CountersImpl mapCounters) throws IOException {
      Preconditions.checkArgument(inputs.size() == outputs.size(),
          "%s reduce inputs, %s outputs", inputs.size(), outputs.size());
      ...
      for (int shard = 0; shard < outputs.size(); shard++) {
        tasks.add(
            new ReduceShardTask<K, V, O>(id, shard, outputs.size(),
      ...
      final CountersImpl[] counters = new CountersImpl[1];
      WorkerResult<O> result = InProcessShardedJobRunner.runJob(tasks.build(),
          new AbstractWorkerController<KeyValue<K, ReducerInput<V>>, O, ReducerContext<O>>(
              mrSpec.getJobName() + "-reduce") {
            // Not really meant to be serialized, but avoid warning.
            private static final long serialVersionUID = 575338448598450119L;

            @Override public void completed(WorkerResult<O> result) {
  ...
  public static <I, K, V, O, R> MapReduceResult<R> runMapReduce(
      ...
      throws IOException {
    String mapReduceId = "in-process-mr-" + DATE_FORMAT.format(new Date()) + "-"
    ...
    MapResult<K, V> mapResult = mapReduce.map(inputs);

    List<? extends OutputWriter<O>> outputs = mrSpec.getOutput().createWriters();
    List<List<KeyValue<K, List<V>>>> reducerInputs = mapReduce.shuffle(
        mapResult.getMapShardOutputs(), outputs.size());
    MapReduceResult<R> result = mapReduce.reduce(reducerInputs, mrSpec.getOutput(), outputs,
