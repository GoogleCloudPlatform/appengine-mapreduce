...
abstract class BaseDatastoreInput<V, R extends BaseDatastoreInputReader<V>> extends Input<V> {
   ...
   */
  public BaseDatastoreInput(String entityKind, int shardCount) {
   ...
   * @param shardCount number of parallel shards for the input.
   */
  public BaseDatastoreInput(String entityKind, int shardCount, String namespace) {
  ...
  private void addInputReader(List<R> result, Key start, Key end) {
    R source = createReader(entityKind, start, end, namespace);
